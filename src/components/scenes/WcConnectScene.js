...
export const WcConnectScene = (props: Props) => {
  ...
  const { walletName, wallet, currencyWallets, contractAddress } = useSelector(state => {
    const { currencyWallets } = state.core.account
    const guiWallet = getSelectedWallet(state)
    const wallet = currencyWallets[guiWallet.id]
    const { metaTokens } = wallet.currencyInfo
  ...
  const renderWalletSelect = () => {
    if (selectedWallet.walletId === '' && selectedWallet.currencyCode === '') {
      ...
      const walletImage = <CurrencyIcon tokenId={contractAddress} walletId={selectedWallet.walletId} resizeMode="contain" />
