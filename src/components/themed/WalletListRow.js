...
type Props = {
  ...
  exchangeRateType?: null | string, // Should be any of this values: 'neutral' | 'positive' | 'negative'
  children?: React.Node,
  icon?: React.Node,
  iconUri?: string,
  iconSize?: number,
...
export const WalletListRowComponent = (props: Props) => {
  const { currencyCode, children, gradient = false, icon, iconUri = '', iconSize, editIcon, onPress, onLongPress, walletName = '' } = props
  const theme = useTheme()
  const styles = getStyles(theme)

  const iconSizeStyle = iconSize != null ? { width: theme.rem(iconSize), height: theme.rem(iconSize) } : styles.iconSize
