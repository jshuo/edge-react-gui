// @flow

// Top-level router:
export const ROOT = 'root'
export const LOGIN = 'login'
export const EDGE = 'edge'

// Logged-in scenes:
export const CHANGE_MINING_FEE = 'changeMiningFee'
export const CHANGE_PASSWORD = 'changePassword'
export const CHANGE_PIN = 'changePin'
export const CONNECTBLE = 'ScanConnectSecux'
export const CREATE_WALLET_ACCOUNT_SELECT = 'createWalletAccountSelect'
export const CREATE_WALLET_ACCOUNT_SETUP = 'createWalletAccountSetup'
export const CREATE_WALLET_CHOICE = 'createWalletChoice'
export const CREATE_WALLET_IMPORT = 'createWalletImport'
export const CREATE_WALLET_NAME = 'createWalletName'
export const CREATE_WALLET_REVIEW = 'createWalletReview'
export const CREATE_WALLET_SELECT_CRYPTO = 'createWalletSelectCrypto'
export const CREATE_WALLET_SELECT_FIAT = 'createWalletSelectFiat'
export const CURRENCY_SETTINGS = 'currencySettings'
export const CURRENCY_NOTIFICATION_SETTINGS = 'currencyNotificationSettings'
export const DEFAULT_FIAT_SETTING = 'defaultFiatSetting'
export const EDGE_LOGIN = 'edgeLogin'
export const EXCHANGE = 'exchange'
export const EXCHANGE_QUOTE_PROCESSING_SCENE = 'exchangeQuoteProcessing'
export const EXCHANGE_QUOTE_SCENE = 'exchangeQuote'
export const EXCHANGE_SUCCESS_SCENE = 'exchangeSuccess'
export const EXCHANGE_SCENE = 'exchangeScene'
export const EXCHANGE_SETTINGS = 'exchangeSettings'
export const FIO_NAME_CONFIRM = 'fioNameConfirm'
export const FIO_ADDRESS_DETAILS = 'fioAddressDetails'
export const FIO_ADDRESS_LIST = 'fioAddressList'
export const FIO_ADDRESS_REGISTER = 'fioAddressRegister'
export const FIO_ADDRESS_REGISTER_SELECT_WALLET = 'fioAddressRegisterSelectWallet'
export const FIO_ADDRESS_REGISTER_SUCCESS = 'fioAddressRegisterSuccess'
export const FIO_REQUEST_LIST = 'fioRequestList'
export const FIO_REQUEST_APPROVED = 'fioRequestApproved'
export const FIO_SENT_REQUEST_DETAILS = 'fioSentRequestDetails'
export const FIO_REQUEST_CONFIRMATION = 'fioRequestConfirmation'
export const FIO_CONNECT_TO_WALLETS_CONFIRM = 'fioConnectToWalletsConfirm'
export const FIO_ADDRESS_SETTINGS = 'fioAddressSettings'
export const FIO_DOMAIN_SETTINGS = 'fioDomainSettings'
export const FIO_DOMAIN_REGISTER = 'fioDomainRegister'
export const FIO_DOMAIN_REGISTER_SELECT_WALLET = 'fioDomainRegisterSelectWallet'
export const FIO_DOMAIN_CONFIRM = 'fioDomainConfirm'
export const NOTIFICATION_SETTINGS = 'notificationSettings'
export const OTP_REPAIR_SCENE = 'otpRepair'
export const OTP_SETUP = 'otpSetup'
export const PLUGIN_BUY = 'pluginBuy'
export const PLUGIN_SELL = 'pluginSell'
export const PLUGIN_VIEW = 'pluginView'
export const PROMOTION_SETTINGS = 'promotionSettings'
export const RECOVER_PASSWORD = 'passwordRecovery'
export const REQUEST = 'request'
export const SECURITY_ALERTS_SCENE = 'securityAlerts'
export const SEND = 'send'
export const SETTINGS_OVERVIEW = 'settingsOverview'
export const SETTINGS_OVERVIEW_TAB = 'settingsOverviewTab'
export const SPENDING_LIMITS = 'spendingLimits'
export const STAKE_MODIFY = 'stakeModify'
export const STAKE_OPTIONS = 'stakeOptions'
export const STAKE_OVERVIEW = 'stakeOverview'
export const FIO_STAKING_CHANGE = 'fioStakingChange'
export const FIO_STAKING_OVERVIEW = 'fioStakingOverview'
export const TERMS_OF_SERVICE = 'termsOfService'
export const TRANSACTION_DETAILS = 'transactionDetails'
export const TRANSACTION_LIST = 'transactionList'
export const TRANSACTIONS_EXPORT = 'transactionsExport'
export const WALLET_LIST = 'walletList'
export const WALLET_LIST_SCENE = 'walletListScene' // distinguished as actual scene vs. stack
